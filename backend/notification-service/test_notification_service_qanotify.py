...
async def test_error_handling(sender_service):
    """
    测试错误处理
    ...
    try:
        ...
        try:
            await sender_service._send_qanotify(channel, notification, content)
            print("❌ 应该抛出token错误但没有")
            return False
        except Exception as e:
            if "token not configured" in str(e).lower():
                print("✅ 正确处理了token缺失错误")
                return True
            else:
                print(f"❌ 错误类型不正确: {e}")
                return False
...
async def main():
    ...
    # 6. 测试错误处理
    error_test = await test_error_handling(sender_service)
